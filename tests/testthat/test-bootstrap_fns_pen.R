...
test_that("lasso 1se ncvreg  works", {
  expect_no_error({
    set.seed(1)
    sel <- select_ncvreg(Sepal.Length ~ ., iris, penalty ="lasso", alpha=1,lambda = "compact")
    inf <- infer_boot(sel, data = iris, B = 20, debias = FALSE,  inference_target ="selections")
    vals <- tidy(inf)
    capture_output(print(inf))
...
test_that("mcp 1se ncvreg  works", {
  expect_no_error({
    set.seed(1)
    sel <- select_ncvreg(Sepal.Length ~ ., iris, penalty ="MCP", alpha=1,lambda = "compact")
    inf <- infer_boot(sel, data = iris, B = 20, debias = FALSE,  inference_target ="selections")
    vals <- tidy(inf)
    capture_output(print(inf))
...
test_that("lasso 1se glmnet  works", {
  expect_no_error({
    set.seed(1)
    sel <- select_glmnet(Sepal.Length ~ ., iris, lambda = "compact")
    inf <- infer_boot(sel, data = iris, B = 20, debias = FALSE,  inference_target ="selections")
    vals <- tidy(inf)
    capture_output(print(inf))
...
test_that("elastic net compact  works", {
  expect_no_error({
    set.seed(1)
    sel <- select_glmnet(Sepal.Length ~ ., iris, lambda = "compact", alpha=0.5)
    inf <- infer_boot(sel, data = iris, B = 20, debias = FALSE,  inference_target ="selections")
    vals <- tidy(inf)
    capture_output(print(inf))

...
test_that("Lasso min works ", {
  expect_no_error({
    ...
    obj2ns <-infer(lasso_mod_nostd , method = "boot", B=5, parallel=T)
    obj3ns <- infer(lasso_mod_nostd, method = "boot", B=5, nonselection = "confident_nulls")
    obj4ns <- infer(lasso_mod_nostd , method = "boot", B=5, nonselection = "confident_nulls",parallel=T)
    obj5ns <-infer(lasso_mod_nostd , method = "boot", B=5, nonselection = "uncertain_nulls")
    obj6ns <-infer(lasso_mod_nostd , method = "boot", B=5,  nonselection = "uncertain_nulls",parallel=T)
...
test_that("MCP min works ", {
  expect_no_error({
    ...
    obj2s <-infer(mcp_mod, method = "boot", B=5, parallel=T)
    obj3s <- infer(mcp_mod, method = "boot", B=5, nonselection = "confident_nulls")
    obj4s <- infer(mcp_mod, method = "boot", B=5, nonselection = "confident_nulls",parallel=T)
    obj5s <-infer(mcp_mod, method = "boot", B=5, nonselection = "uncertain_nulls")
    obj6s <-infer(mcp_mod, method = "boot", B=5, nonselection = "uncertain_nulls",parallel=T)


    mcp_mod_nostd <- pen_cv (x=x,y=y,penalty= "MCP",lambda="lambda.min",std=FALSE)
    obj1ns <- infer(mcp_mod_nostd,  method = "boot", B=5)
    obj2ns <-infer(mcp_mod_nostd , method = "boot", B=5, parallel=T)
    obj3ns <- infer(mcp_mod_nostd, method = "boot", B=5, nonselection = "confident_nulls")
    obj4ns <- infer(mcp_mod_nostd , method = "boot", B=5, nonselection = "confident_nulls",parallel=T)
    obj5ns <-infer(mcp_mod_nostd , method = "boot", B=5, nonselection = "uncertain_nulls")
    obj6ns <-infer(mcp_mod_nostd , method = "boot", B=5,  nonselection = "uncertain_nulls",parallel=T)

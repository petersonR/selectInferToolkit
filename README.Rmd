...
* **Penalized Regression:** Easily perform lasso, ridge, and elastic net regressions using the powerful `ncvreg` or `glmnet` packages. `selectInferToolkit` simplifies the process of obtaining models associated with either $\lambda_{min}$ or $\lambda_{1se}$.
...
1.  **Hybrid Ordinary Least Squares:** A baseline approach that performs no adjustment for the selection process. We refer to this as "unadjusted post-selection inference", or "UPSI" for short. 
...
A unique and important feature of `selectInferToolkit` is its flexibility in handling non-selected variables (those not included in the final model). For each post-selection inference method, we offer three distinct approaches:
...
```{r AIC selectiveinference, eval = FALSE}
...
With the `uncertain_nulls` approach, we see that the `cyl` and `wt` both have CIs which do not intersect zero, indicating significant effects.
